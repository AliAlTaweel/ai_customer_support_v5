...
from services.kb_document_service import KBDocumentService
from services.kb_qa_service import KBQAService
...
async def upload_document(request: Request, file: UploadFile = File(...)):
    ...
    result = await KBDocumentService.upload_document(tenant_id, file.filename, file_bytes)
...
async def list_documents(request: Request):
    tenant_id = request.state.tenant_id
    documents = await KBDocumentService.list_documents(tenant_id)
...
async def delete_document(request: Request, document_id: str):
    tenant_id = request.state.tenant_id
    deleted = await KBDocumentService.delete_document(tenant_id, document_id)
    if not deleted:
...
async def create_qa_pair(request: Request, req: QAPairCreate):
    tenant_id = request.state.tenant_id
    result = await KBQAService.create_qa_pair(tenant_id, req.question, req.answer)
...
async def list_qa_pairs(request: Request):
    tenant_id = request.state.tenant_id
    qa_pairs = await KBQAService.list_qa_pairs(tenant_id)
...
async def update_qa_pair(request: Request, qa_id: str, req: QAPairUpdate):
    tenant_id = request.state.tenant_id
    result = await KBQAService.update_qa_pair(tenant_id, qa_id, req.question, req.answer)
    if not result:
...
async def delete_qa_pair(request: Request, qa_id: str):
    tenant_id = request.state.tenant_id
    deleted = await KBQAService.delete_qa_pair(tenant_id, qa_id)
    if not deleted:
        raise HTTPException(status_code=404, detail="Q&A pair not found")
...
async def update_ai_settings(request: Request, req: AISettingsRequest):
    tenant_id = request.state.tenant_id
    ai_enabled = await TenantSettingsService.set_ai_enabled(tenant_id, req.ai_enabled)
    return AISettingsResponse(ai_enabled=ai_enabled)
